...
def main():
    ...
    try:
        ...
        while True:
            ...
            print(f"EPH: {formatted_eph_data}") if DEBUG else None

            formatted_pr_data = format_pseudorange(pr_data)
            print(f"PR: {formatted_pr_data}") if DEBUG else None

            receiver_tow = format_receiver_tow(pr_data)
            print(f"Receiver TOW: {receiver_tow}") if DEBUG else None
            ...
            if formatted_pr_data.keys() == formatted_eph_data.keys() and len(formatted_eph_data) > 4:
                ...
                print(f"Sat CLK Corrected PR: {formatted_corrected_pseudorange_data}") if DEBUG else None
                ...
                while np.linalg.norm(dx) > 0.1 and abs(db) > 1:
                    ...
                    for svid, pseudorange in formatted_corrected_pseudorange_data.items():
                        ...
                        print(f"Satellite {svid} ECEF Position: {ds_}") if DEBUG else None
                        theta = OMEGA_E * tau
                    ...
                    print(f"Xs: {Xs}") if DEBUG else None
                    print(f"pr: {pr}") if DEBUG else None
                    ...
                    xu = x_
                    b = b_
                    print(f"xu: {xu}") if DEBUG else None
                    print(f"norm_dp: {norm_dp}") if DEBUG else None
                    print(f"G: {G}") if DEBUG else None
